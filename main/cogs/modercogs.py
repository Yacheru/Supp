...
class ModerCommandsCog(commands.Cog):
    ...
    try:
        ...
        async def clear(self, inter: discord.Interaction, count: int):
            ...
            if len(msg) == 0:
                embed_clear = discord.Embed(title="", description=f"Ничего не удалено c <#{inter.channel.id}>",
                                            timestamp=datetime.datetime.now(), color=discord.Colour.green())
            else:
                embed_clear = discord.Embed(title="",
                                            description=f"Успешно удалено **{len(msg)}** сообщений c <#{inter.channel.id}>",
                                            timestamp=datetime.datetime.now(), color=discord.Colour.green())

            embed_clear.set_author(name=f"{inter.user.name}#{inter.user.discriminator}",
                                   icon_url=f"{inter.user.avatar.url}")
        ...
        async def mute(self, inter: discord.Interaction, user: discord.Member, time: str, reason: str):
            ...
            embed_timeout = discord.Embed(title="<:timeout:1113547982764261438> Добавление Тайм-аута",
            ...
            try:
                ...
                await inter.response.send_message(
                    f"Пользователь {user.mention} был отправлен подумать о своём поведении", ephemeral=True)
        ...
        async def unmute(self, inter: discord.Interaction, user: discord.Member):
            ...
            embed_timeout = discord.Embed(title="<:untimeout:1113584329130524712> Снятие Тайм-аута",
            ...
            try:
                ...
                print(e)
                await inter.response.send_message(
                    f"Произошла ошибка при снятии наказания с пользователя {user.mention}({user.id})", ephemeral=True)
        ...
        async def kick(self, inter: discord.Interaction, user: discord.Member, reason: str):
            ...
            embed_kick = discord.Embed(title="<:ban:1113647045861978162> Кик", color=discord.Colour.red(),
            ...
            try:
                ...
                print(e)
                await inter.response.send_message(f"Произошла ошибка при кике пользователя {user.mention}({user.id})",
                                                  ephemeral=True)
        ...
        async def ban(self, inter: discord.Interaction, user: discord.Member, reason: str):
            if user:
                ...
                embed_ban = discord.Embed(title="<:ban:1113647045861978162> Бан", color=discord.Colour.red(),
                                          timestamp=datetime.datetime.now())
                ...
                try:
                    ...
                    print(e)
                    await inter.response.send_message(
                        f"Произошла ошибка при бане пользователя {user.mention}({user.id})", ephemeral=True)
        ...
        async def warn(self, inter: discord.Interaction, user: discord.Member, reason: str):
            ...
            pcursor.execute("SELECT reason FROM moderation WHERE user_id = %s AND guild_id = %s",
        ...
        async def warnings(self, inter: discord.Interaction, user: discord.Member):
            ...
            em_warnings = Embed(title="Предупреждения пользователя:", timestamp=datetime.datetime.now(),
                                color=Colour.yellow())
        ...
        async def clear_warnings(self, inter: discord.Interaction, user: discord.Member):
            ...
            pcursor.execute("SELECT reason FROM moderation WHERE user_id = ? AND guild_id = ?",
                            (user.id, inter.guild.id))
            result = pcursor.fetchall()

            em_clear_warns = Embed(title="Очистка предупреждений", color=discord.Colour.green(),
                                   timestamp=datetime.datetime.now())
        ...
        async def all_warnings(self, inter: discord.Interaction):
            ...
            if results:
                ...
                em_all_warnings = Embed(title="Список всех предупреждений", timestamp=datetime.datetime.now(),
                                        color=Colour.blue())
                for user, reasons in user_reasons.items():
                    reasons_text = '\n'.join(
                        [f"Предупреждение: {reason}\nДата: `{date}`\n" for reason, date in reasons])
