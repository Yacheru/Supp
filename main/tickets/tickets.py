...
async def ticket_create(inter: discord.Interaction, ticket: str):
    ...
    em = Embed(title=f"{ticket}",
               description=f"Тикет на тему: **{ticket}** *создан* пользователем: {inter.user.mention}",
               color=Colour.from_rgb(88, 101, 242) if ticket == 'Жалоба на игрока' else Colour.from_rgb(78, 80, 88),
    ...
    def __init__(self):
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            await asyncio.gather(
                ...
                inter.message.edit(embed=(inter.message.embeds[0].set_footer(text=f"Тикет закрыт: {inter.user.name}")),
                                   view=DeleteTicket()))
...
class Usercomplaints(Modal, title="Жалоба на игрока"):
    member = TextInput(label="Ссылка на игрока:", placeholder="Укажите Steam-профиль нарушителя", style=TextStyle.short,
                       max_length=500)
    docs = TextInput(label="Доказательства:", placeholder="Укажите ссылку на фото/видео доказательство нарушения",
                     style=TextStyle.short, max_length=500)
    description = TextInput(label="Что нарушил игрок:",
                            placeholder="Опишите, какое правило нарушил игрок: https://infinity-tm.ru/pages/rules",
                            style=TextStyle.paragraph, max_length=2000)
    ...
    async def on_submit(self, inter: discord.Interaction):
        ...
        embed_complaints = Embed(title=f"{self.title}",
                                 description=f"**Комментарий заявителя:**\n>>> {self.description.value}",
                                 color=Colour.dark_theme(), timestamp=datetime.now())
        embed_complaints.add_field(name="\u200b",
                                   value=f"- **Информация:**\n - {self.member.value} *(игрок)*\n - {self.docs.value} *(доказательства)*",
                                   inline=True)
        embed_complaints.set_author(name=f"{inter.user.name}",
        ...
        embed_complaints.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/1129601347352809532/1148473215568588893/player_complaints.png")
        embed_complaints.set_image(
            url="https://cdn.discordapp.com/attachments/1111378209934680087/1113885628132765747/invs.png")
        ...
        channel = await inter.guild.create_text_channel(name=f"Обращение: {inter.user.name}", category=open,
                                                        overwrites=overwrites)
        await channel.send(content="||<@&-860197500735586325>, <@&-1146225687070523453>||", embed=embed_complaints,
                           view=CloseTicket())
...
class Support(Modal, title="По любым вопросам"):
    ...
    async def on_submit(self, inter: discord.Interaction):
        ...
        embed_complaints.set_footer(text="Нажмите на кнопку ниже, чтобы закрыть обращение.")
        embed_complaints.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/1129601347352809532/1149616464718987375/a137c994eeb9f6f2.png")
        embed_complaints.set_image(
            url="https://cdn.discordapp.com/attachments/1111378209934680087/1113885628132765747/invs.png")
...
class ButtonTicket(View):
    def __init__(self):
        ...
        async def button_callback(inter: discord.Interaction):
            await inter.response.send_modal(Usercomplaints())
...
async def sup(ctx: commands.Context):
    ...
    embed_support_image.set_image(
        url="https://cdn.discordapp.com/attachments/1129601347352809532/1148476569342451752/infinity_support.png")
    ...
    embed_support.set_image(
        url="https://cdn.discordapp.com/attachments/1111378209934680087/1113885628132765747/invs.png")
    embed_support.add_field(name="",
                            value="При создании тикета необходимо предоставлять как можно больше подробной информации, включая видео/скриншоты, ссылки на доказательства и прочее, что может способствовать быстрому рассмотрению тикета\n\nПри создании тикета учитывайте время, в которое вы пишете. Время ответа на ваш тикет зависит от сложности обращения и загруженности администратции.\n\n- [**Жалобы на администраторов**](https://infinity-tm.ru/complaints/)\n- [**Заявки на разбан**](https://infinity-tm.ru/bans/)",
                            inline=False)
