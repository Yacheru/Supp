...
async def on_message(message: discord.Message):
    try:
        ...
        if user_data:
            pcursor.execute("UPDATE messages_activity SET messages_count = messages_count + 1 WHERE userID = %s",
                            (message.author.id,))
        else:
            pcursor.execute("INSERT INTO messages_activity (userID, messages_count) VALUES (%s, 1)",
                            (message.author.id,))
...
async def update_most_active_user_data():
    try:
        ...
        if user_data is not None and user_data[1] != 0:
            ...
            if most_active:
                nickname = most_active.display_name[:7] + "..." if len(
                    most_active.display_name) > 7 else most_active.display_name
                avatar_url = most_active.avatar.url if most_active.avatar else most_active.default_avatar.url

                image(url=avatar_url)

                if result is None:
                    pcursor.execute("INSERT INTO most_msg_activity_user (nickname, messages_count) VALUES (%s, %s)",
                                    (nickname, user_data[1]))
                else:
                    pcursor.execute(
                        "UPDATE most_msg_activity_user SET nickname = %s, messages_count = %s WHERE nickname = %s",
                        (nickname, user_data[1], result[0]))

                print(
                    f"[{datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}] [UPDATE ACTIVE USER] [TASK] [INFO] MOST ACTIVE USER INSERTED AT")
            else:
                pcursor.execute(
                    "UPDATE most_msg_activity_user SET nickname = %s, messages_count = %s WHERE nickname = %s",
                    ("Неизвестно", 0, result[0] if result else None))
        else:
            pcursor.execute("UPDATE most_msg_activity_user SET nickname = %s, messages_count = %s WHERE nickname = %s",
                            ("Неизвестно", 0, result[0] if result else None))

        print(
            f"[{datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}] [UPDATE ACTIVE USER] [TASK] [INFO] CAN'T INSERT MOST ACTIVE USER AT")
    except Exception as e:
        print(f"[{datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}] [UPDATE ACTIVE USER] [TASK] [ERROR] WITH CODE",
              e)
...
async def update_banner():
    try:
        ...
        print(
            f"[{datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}] [UPDATE GUILD BANNER] [TASK] [INFO] BANNER SUCCESSFULLY UPDATED AT")
    except Exception as e:
        print(f"[{datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}] [UPDATE GUILD BANNER] [TASK] [ERROR] WITH CODE",
              e)
