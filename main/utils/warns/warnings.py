...
class WarnModal(Modal, title = "Добавить предупреждение"):
    steamid = TextInput(label="Steamid админа:", style=TextStyle.short, placeholder="Формат: STEAM_X:X:XXXXXXXXX", max_length=23)
    inflink = TextInput(label="Профиль админа:", style=TextStyle.short, placeholder="https://infinity-tm.ru/profile/GoddamN", max_length=100)
    docs = TextInput(label="Доказательства на нарушение:", style=TextStyle.short, placeholder="Вставьте ссылку на видео/фото док-во нарушения", required=False, max_length=500)
    warn = TextInput(label="Причина выдачи варна:", style=TextStyle.long, placeholder="Напишите здесь причину варна", max_length=1000)
        ...
        if result:
            if result[1] == inter.user.id:
                ...
                if result:
                    ...
                    await inter.response.send_message(content=f"Не найдено предупреждений для {steamid}", ephemeral=True)
            else:
                await inter.response.send_message("Удалить предупреждение может только тот, кто его выдал.", ephemeral=True)
        else:
...
class FindModal(Modal, title = "Найти предупреждение"):
    ...
    async def on_submit(self, inter: discord.Interaction):
        ...
        if result:
            em = Embed(title="", description="### Найденные предупреждения:", color=Colour.red(), timestamp=datetime.now())
            for i, values in enumerate(result, start=1):
...
class Confirm(View):
    def __init__(self):
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            pcursor.execute("INSERT INTO infwarns (giving, steam_id, inf_link, warn, docs, date) VALUES (%s, %s, %s, %s, %s, %s)", (inter.user.id, steamid_value, inflink_value, warn_value, docs_value, format_dt(datetime.now(), style='D')))
...
async def warn(ctx: commands.Context):
    ...
    em_img.set_image(url="https://cdn.discordapp.com/attachments/1129601347352809532/1152052676868182027/00e7f17c937e95bc.png")

    em = Embed(title="Предупреждения Администраторов", description="Первая кнопка позволит вам отправить предупреждение администратору, а вторая кнопка поможет вам найти уже существующее предупреждение.", color=Colour.red())
